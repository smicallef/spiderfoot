...
class sfp_circllu(SpiderFootPlugin):
    ...
    def query(self, qry, qtype):
        ...
        if res['code'] not in ["200", "201"]:
            self.log.error("CIRCL.LU access seems to have been rejected or you have exceeded usage limits.")
            self.errorState = True
            return None

        if res['content'] is None:
            self.log.info("No CIRCL.LU info found for " + qry)
            return None
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key_login'] == "" or self.opts['api_key_password'] == "":
            self.log.error("You enabled sfp_circllu but did not set an credentials!")
            self.errorState = True
            return
        ...
        if eventName in ['IP_ADDRESS', 'NETBLOCK_OWNER']:
            ...
            if "/" in eventData:
                ...
                if int(mask) < 23:
                    self.log.debug("Network size bigger than permitted by CIRCL.LU.")
                else:
                    ret = self.query(eventData, "PSSL")
                    if not ret:
                        self.log.info("No CIRCL.LU passive SSL data found for " + eventData)
            else:
                ret = self.query(eventData, "PSSL")
                if not ret:
                    self.log.info("No CIRCL.LU passive SSL data found for " + eventData)
            ...
            if ret:
                try:
                    ...
                    self.log.error("Invalid response returned from CIRCL.LU: " + str(e))

        if eventName in ['IP_ADDRESS', 'INTERNET_NAME', 'DOMAIN_NAME']:
            ret = self.query(eventData, "PDNS")
            if not ret:
                self.log.info("No CIRCL.LU passive DNS data found for " + eventData)
                return
                ...
                for co in cohosts:
                    ...
                    if not self.opts['cohostsamedomain']:
                        if self.getTarget().matches(co, includeParents=True):
                            self.log.debug("Skipping " + co + " because it is on the same domain.")
                            continue
