...
class sfp_openstreetmap(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        if res['content'] is None:
            self.log.info("No location info found for " + qry)
            return None
    ...
    def handleEvent(self, event):
        ...
        if address.lower().startswith('po box'):
            self.log.debug("Skipping PO BOX address")
            return
        ...
        self.log.info("Found " + str(len(data)) + " matches for " + eventData)
        ...
        for location in data:
            try:
                ...
                self.log.debug("Failed to get lat/lon: " + str(e))
                continue
            ...
            self.log.debug("Found coordinates: " + coords)
