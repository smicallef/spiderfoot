...
class sfp_wigle(SpiderFootPlugin):
    ...
    def getcoords(self, qry):
        ...
        if "too many queries" in res['content']:
            self.log.error("Wigle.net query limit reached for the day.")
            return None
        ...
        try:
            ...
            self.log.error(f"Error processing JSON response from Wigle.net: {e}")
            return None
    ...
    def getnetworks(self, coords):
        ...
        try:
            ...
            self.log.error(f"Error processing JSON response from WiGLE: {e}")
            return None
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key_encoded'] == "":
            self.log.error("You enabled sfp_wigle but did not set an API key!")
        ...
        if not coords:
            self.log.error("Couldn't get coordinates for address from Wigle.net.")
            return

        nets = self.getnetworks(coords)
        if not nets:
            self.log.error("Couldn't get networks for coordinates from Wigle.net.")
            return
