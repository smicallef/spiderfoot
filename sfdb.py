...
class SpiderFootDb:
    ...
    def scanLogs(self, instanceId, limit=None, fromRowId=None, reverse=False):
        ...
        if not isinstance(instanceId, str):
            raise TypeError("instanceId is {type(instanceId)}; expected str()")
    ...
    def scanEventStore(self, instanceId, sfEvent, truncateSize=0):
        ...
        if not isinstance(sfEvent.eventType, str):
            raise TypeError("sfEvent.eventType is {type(sfEvent.eventType,)}; expected str()")

        if not sfEvent.eventType:
            raise ValueError("sfEvent.eventType is empty")

        if not isinstance(sfEvent.data, str):
            raise TypeError("sfEvent.data is {type(sfEvent.data)}; expected str()")

        if not sfEvent.data:
            raise ValueError("sfEvent.data is empty")

        if not isinstance(sfEvent.module, str):
            raise TypeError("sfEvent.module is {type(sfEvent.module)}; expected str()")
        ...
        if not isinstance(sfEvent.confidence, int):
            raise TypeError("sfEvent.confidence is {type(sfEvent.confidence)}; expected int()")

        if not 0 <= sfEvent.confidence <= 100:
            raise ValueError("sfEvent.confidence value is {type(sfEvent.confidence)}; expected 0 - 100")

        if not isinstance(sfEvent.visibility, int):
            raise TypeError("sfEvent.visibility is {type(sfEvent.visibility)}; expected int()")

        if not 0 <= sfEvent.visibility <= 100:
            raise ValueError("sfEvent.visibility value is {type(sfEvent.visibility)}; expected 0 - 100")

        if not isinstance(sfEvent.risk, int):
            raise TypeError("sfEvent.risk is {type(sfEvent.risk)}; expected int()")

        if not 0 <= sfEvent.risk <= 100:
            raise ValueError("sfEvent.risk value is {type(sfEvent.risk)}; expected 0 - 100")

        if not isinstance(sfEvent.sourceEvent, SpiderFootEvent):
            if sfEvent.eventType != "ROOT":
                raise TypeError("sfEvent.sourceEvent is {type(isfEvent.sourceEvent)}; expected str()")

        if not isinstance(sfEvent.sourceEventHash, str):
            raise TypeError("sfEvent.sourceEventHash is {type(sfEvent.sourceEventHash)}; expected str()")
