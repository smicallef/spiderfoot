    ...
    def handleEvent(self, event):
        ...
        if eventName in ["INTERNET_NAME", "DOMAIN_NAME"]:
            ...
            if google_adsense:
                for r in list(google_adsense.keys()):
                    evt = SpiderFootEvent(
                        "WEB_ANALYTICS_ID", "Google AdSense: " + r, self.__name__, event
                    )
            ...
            if google_analytics:
                for r in list(google_analytics.keys()):
                    evt = SpiderFootEvent(
                        "WEB_ANALYTICS_ID", "Google Analytics: " + r, self.__name__, event
                    )
                    self.notifyListeners(evt)
        ...
        if eventName in ["WEB_ANALYTICS_ID"]:
            ...
            for r in list(data.keys()):
                last_seen = (
                    int(datetime.datetime.strptime(data[r], "%Y-%m-%d").strftime("%s")) * 1000
                )
        ...
        if eventName in ["IP_ADDRESS"]:
            ...
            for co in list(data.keys()):
                last_seen = (
                    int(datetime.datetime.strptime(data[co], "%Y-%m-%d").strftime("%s")) * 1000
                )
