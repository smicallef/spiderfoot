...
class sfp_threatfox(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        if res['code'] == "429":
            self.log.error("You are being rate-limited by ThreatFox.")
            self.errorState = True
            return None

        if res['code'] != '200':
            self.log.error(f"Unexpected reply from ThreatFox: {res['code']}")
            self.errorState = True
            return None
        ...
        try:
            ...
            self.log.debug(f"Error processing JSON response from ThreatFox: {e}")
            return None

        query_status = json_result.get('query_status')

        if query_status == 'no_result':
            self.log.debug(f"No results from ThreatFox for: {qry}")
            return None

        if query_status != 'ok':
            self.log.debug(f"ThreatFox query failed: {query_status}")
            return None
