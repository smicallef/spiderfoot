...
class sfp_whoxy(SpiderFootPlugin):
    ...
    def query(self, qry, querytype, page=1, accum=None):
        ...
        if res['code'] in ["400", "429", "500", "403"]:
            self.log.error("Whoxy API key seems to have been rejected or you have exceeded usage limits.")
            self.errorState = True
            return None

        if res['content'] is None:
            self.log.info("No Whoxy info found for " + qry)
            return None

        try:
            info = json.loads(res['content'])
            if info.get("status", 0) == 0:
                self.log.error("Error querying Whoxy: " + info.get("status_reason", "Unknown"))
                self.errorState = True
                return None
            ...
            self.log.error("Error processing JSON response from Whoxy: " + str(e))
            return None
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.log.error("You enabled sfp_whoxy but did not set an API key!")
            self.errorState = True
            return
