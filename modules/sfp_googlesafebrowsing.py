...
class sfp_googlesafebrowsing(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        if res["code"] == "400":
            self.log.error("Invalid request payload on Google Safe Browsing API")
            self.errorState = True
            return None

        if res["code"] == "429":
            self.log.error("Reaching rate limit on Google Safe Browsing API")
            self.errorState = True
            return None
        ...
        try:
            ...
            if info == {}:
                self.log.info("No Google Safe Browsing matches found for " + qry)
                return None
