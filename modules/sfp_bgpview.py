...
class sfp_bgpview(SpiderFootPlugin):
    ...
    def queryAsn(self, qry):
        ...
        try:
            ...
            self.log.debug(f"Error processing JSON response from BGPView: {e}")
            return None

        if json_data.get('status') != 'ok':
            self.log.debug("No results found for ASN " + qry)
            return None
    ...
    def queryIp(self, qry):
        ...
        if json_data.get('status') != 'ok':
            self.log.debug("No results found for IP address " + qry)
            return None
    ...
    def queryNetblock(self, qry):
        ...
        if json_data.get('status') != 'ok':
            self.log.debug("No results found for netblock " + qry)
            return None
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'BGP_AS_MEMBER':
            ...
            if not data:
                self.log.info("No results found for ASN " + eventData)
                return
        ...
        if eventName in ['NETBLOCK_MEMBER', 'NETBLOCKV6_MEMBER']:
            ...
            if not data:
                self.log.info("No results found for netblock " + eventData)
                return
        ...
        if eventName in ['IP_ADDRESS', 'IPV6_ADDRESS']:
            ...
            if not data:
                self.log.info("No results found for IP address " + eventData)
                return
            ...
            if not prefixes:
                self.log.info("No prefixes found for IP address " + eventData)
                return
            ...
            for prefix in prefixes:
                ...
                self.log.info(f"Netblock found: {p} ({asn})")
