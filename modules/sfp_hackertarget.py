...
class sfp_hackertarget(SpiderFootPlugin):
    ...
    def portScanUDP(self, ip):
        ...
        html_data = re.findall(
            r'<pre id="formResponse">(.*?)</pre>', res["content"], re.MULTILINE | re.DOTALL
        )
    ...
    def portScanTCP(self, ip):
        res = self.sf.fetchUrl(
            ...
            postData="theinput="
            + ip
            + "&thetest=tcpscan&name_of_nonce_field=&_wp_http_referer=%2Ftcp-port-scan%2F",
        )
    ...
    def httpHeaders(self, ip):
        res = self.sf.fetchUrl(
            "https://api.hackertarget.com/httpheaders/?q=" + ip,
            useragent=self.opts["_useragent"],
            timeout=self.opts["_fetchtimeout"],
        )
        ...
        if res["content"] is None:
            self.sf.error(
                "Unable to fetch HTTP headers for " + ip + " from HackerTarget.com.", False
            )
            return None
    ...
    def zoneTransfer(self, ip):
        res = self.sf.fetchUrl(
            "https://api.hackertarget.com/zonetransfer/?q=" + ip,
            useragent=self.opts["_useragent"],
            timeout=self.opts["_fetchtimeout"],
        )
        ...
        for ip in qrylist:
            ...
            if self.opts.get("http_headers", True):
                ...
                if http_headers is not None:
                    e = SpiderFootEvent(
                        "WEBSERVER_HTTPHEADERS", json.dumps(http_headers), self.__name__, pevent
                    )
            ...
            if self.opts.get("udp_portscan", True):
                ...
                if udp_ports:
                    for port in udp_ports:
                        e = SpiderFootEvent(
                            "UDP_PORT_OPEN", ip + ":" + port, self.__name__, pevent
                        )
                        self.notifyListeners(e)
            ...
            if self.opts.get("tcp_portscan", True):
                ...
                if tcp_ports:
                    for port in tcp_ports:
                        e = SpiderFootEvent(
                            "TCP_PORT_OPEN", ip + ":" + port, self.__name__, pevent
                        )
                        self.notifyListeners(e)
