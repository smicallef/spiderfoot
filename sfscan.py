...
class SpiderFootScanner(threading.Thread):
    ...
    def __init__(
        self, scanName, scanTarget, targetType, scanId, moduleList, globalOpts, moduleOpts
    ):

        # Initialize the thread
        threading.Thread.__init__(
            self, name="SF_" + scanId + str(random.SystemRandom().randint(100000, 999999))
        )
    ...
    def setStatus(self, status, started=None, ended=None):
        if self.ts is None:
            print(
                ("Internal Error: Status set attempted before " + "SpiderFootScanner was ready.")
            )
            exit(-1)
    ...
    def startScan(self):
        ...
        self.ts.dbh.scanInstanceCreate(self.ts.scanId, self.ts.scanName, self.ts.targetValue)
        ...
        self.ts.dbh.scanConfigSet(
            self.ts.scanId, self.ts.sf.configSerialize(deepcopy(self.ts.config))
        )
        ...
        try:
            ...
            # Set the user agent
            self.ts.config["_useragent"] = self.ts.sf.optValueToData(self.ts.config["_useragent"])

            # Get internet TLDs
            tlddata = self.ts.sf.cacheGet("internet_tlds", self.ts.config["_internettlds_cache"])
            ...
            firstEvent = SpiderFootEvent(
                self.ts.targetType, self.ts.targetValue, "SpiderFoot UI", rootEvent
            )
            ...
            if self.ts.targetType == "INTERNET_NAME":
                if self.ts.sf.isDomain(self.ts.targetValue, self.ts.config["_internettlds"]):
                    firstEvent = SpiderFootEvent(
                        "DOMAIN_NAME", self.ts.targetValue, "SpiderFoot UI", rootEvent
                    )
                    psMod.notifyListeners(firstEvent)
