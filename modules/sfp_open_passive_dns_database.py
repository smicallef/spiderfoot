...
class sfp_open_passive_dns_database(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        if not rows:
            self.log.debug("No passive DNS results for " + qry)
            return None
        ...
        for row in rows:
            ...
            if len(columns) != 7:
                self.log.error("Unexpected number of columns for row. Expected 7, Found " + str(len(columns)))
                continue

            data.append(columns)

        self.log.info("Retrieved " + str(len(data)) + " DNS records")
    ...
    def handleEvent(self, event):
        ...
        if data is None or len(data) == 0:
            self.log.info("No passive DNS data found for " + eventData)
            return
        ...
        for record in data:
            ...
            if answer_type == 'A':
                ...
                if not self.sf.validIP(answer):
                    self.log.debug("Skipping invalid IP address " + answer)
                    continue

                if self.opts['verify'] and not self.sf.validateIP(query, answer):
                    self.log.debug("Host " + query + " no longer resolves to " + answer)
                    continue
            ...
            if answer_type == 'AAAA':
                ...
                if not self.sf.validIP6(answer):
                    self.log.debug("Skipping invalid IPv6 address " + answer)
                    continue
