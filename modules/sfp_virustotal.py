...
class sfp_virustotal(SpiderFootPlugin):
    ...
    def queryIp(self, qry):
        ...
        if res['content'] is None:
            self.log.info(f"No VirusTotal info found for {qry}")
            return None

        try:
            return json.loads(res['content'])
        except Exception as e:
            self.log.error(f"Error processing JSON response from VirusTotal: {e}")
    ...
    def queryDomain(self, qry):
        ...
        if res['code'] == "204":
            self.log.error("Your request to VirusTotal was throttled.")
            return None
    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            if len(info.get('detected_urls', [])) > 0:
                self.log.info(f"Found VirusTotal URL data for {addr}")
