                ...
                try:
                    ...
                    for res in ret:
                        ...
                        if self.getTarget().matches(
                            res["subjectCommonName"], includeChildren=True
                        ):
                            e = SpiderFootEvent(
                                "INTERNET_NAME", res["subjectCommonName"], self.__name__, event
                            )
                            self.notifyListeners(e)
                            if self.sf.isDomain(
                                res["subjectCommonName"], self.opts["_internettlds"]
                            ):
                                e = SpiderFootEvent(
                                    "DOMAIN_NAME", res["subjectCommonName"], self.__name__, event
                                )
                                self.notifyListeners(e)
                except BaseException as e:
