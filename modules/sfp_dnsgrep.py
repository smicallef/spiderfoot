...
class sfp_dnsgrep(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        if res['code'] != '200':
            self.log.debug("Error retrieving search results for " + qry)
            return None

        try:
            return json.loads(res['content'])
        except Exception as e:
            self.log.error(f"Error processing JSON response from DNSGrep: {e}")
    ...
    def handleEvent(self, event):
        ...
        if data is None:
            self.log.info("No DNS records found for " + eventData)
            return
