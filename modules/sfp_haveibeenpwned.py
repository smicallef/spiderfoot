...
class sfp_haveibeenpwned(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        while retry < 2:
            ...
            if res['code'] == "401":
                self.log.error("Failed to authenticate key with HaveIBeenPwned.com.")
        ...
        try:
            ...
            self.log.error(f"Error processing JSON response from HaveIBeenPwned?: {e}")
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.log.error("You enabled sfp_haveibeenpwned but did not set an API key!")
            self.errorState = True
            return
        ...
        if data is not None:
            for n in data:
                try:
                    ...
                    self.log.debug(f"Unable to parse result from HaveIBeenPwned?: {e}")
                    continue
