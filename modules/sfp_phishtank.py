...
class sfp_phishtank(SpiderFootPlugin):
    ...
    def queryBlacklist(self, target):
        ...
        for item in blacklist:
            ...
            if target.lower() in item[1]:
                self.log.debug(f"Host name {target} found in phishtank.com blacklist.")
                return item[0]
    ...
    def retrieveBlacklist(self):
        ...
        if res['code'] != "200":
            self.log.error(f"Unexpected HTTP response code {res['code']} from phishtank.com.")
            self.errorState = True
            return None

        if res['content'] is None:
            self.log.error("Received no content from phishtank.com")
            self.errorState = True
            return None
    ...
    def handleEvent(self, event):
        ...
        self.log.debug(f"Checking maliciousness of {eventData} ({eventName}) with phishtank.com")
