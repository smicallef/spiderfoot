...
class sfp_emailrep(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        if res['code'] == '400':
            self.log.error('API error: Bad request')
        ...
        if res['code'] != '200':
            self.log.error('Unexpected reply from EmailRep.io: ' + res['code'])
            self.errorState = True
            return None
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == '':
            self.log.error("Warning: You enabled sfp_emailrep but did not set an API key! Queries will be rate limited.")
