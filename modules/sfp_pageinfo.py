...
class sfp_pageinfo(SpiderFootPlugin):
    ...
    def handleEvent(self, event):
        ...
        if "sfp_spider" not in event.module:
            self.log.debug("Ignoring web content from " + event.module)
            return
        ...
        if not self.getTarget().matches(self.sf.urlFQDN(eventSource)):
            self.log.debug("Not gathering page info for external site " + eventSource)
            return

        # Ignore javascript and CSS
        if ".css?" in eventSource or eventSource.endswith(".css"):
            self.log.debug("Not attempting to match CSS content.")
            return

        if ".js?" in eventSource or eventSource.endswith(".js"):
            self.log.debug("Not attempting to match JS content.")
            return

        if eventSource in self.results:
            self.log.debug("Already checked this page for a page type, skipping.")
            return
        ...
        for regexpGrp in regexps:
            ...
            for regex in regexps[regexpGrp]:
                ...
                if len(matches) > 0 and regexpGrp not in self.results[eventSource]:
                    self.log.info("Matched " + regexpGrp + " in content from " + eventSource)
        ...
        if len(self.results[eventSource]) == 0:
            self.log.info("Treating " + eventSource + " as URL_STATIC")
        ...
        if len(matches) > 0:
            for match in matches:
                ...
                if self.getTarget().matches(self.sf.urlFQDN(match)):
                    continue
                self.log.debug(f"Externally hosted JavaScript found at: {match}")
