    ...
    def handleEvent(self, event):
        ...
        if eventName == "EMAILADDR":
            ...
            if res['content'] is None:
                self.log.error(f"Could not fetch MySpace content for {email}")
                return
            ...
            if not profiles:
                self.log.debug(f"No profiles found for e-mail: {email}")
                return
            ...
            try:
                ...
                self.log.debug("Malformed e-mail address, skipping.")
                return

            if not matches:
                self.log.debug("No concrete match for that e-mail.")
                return
        ...
        if eventName == "SOCIAL_MEDIA":
            ...
            if network != "MySpace":
                self.log.debug(f"Skipping social network profile, {url}, as not a MySpace profile")
                return
