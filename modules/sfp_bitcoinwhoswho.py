...
class sfp_bitcoinwhoswho(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        if res["content"] is None:
            self.log.info(f"No {self.meta['name']} info found for {qry}")
            return None

        try:
            return json.loads(res["content"])
        except Exception as e:
            self.log.error(f"Error processing JSON response from {self.meta['name']}: {e}")
    ...
    def handleEvent(self, event):
        ...
        if event.data in self.results:
            self.log.debug(f"Skipping {event.data}, already checked.")
            return
