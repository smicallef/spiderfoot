...
class sfp_github(SpiderFootPlugin):
    ...
    def buildRepoInfo(self, item):
        ...
        name = item.get('name')
        if name is None:
            self.log.debug("Incomplete Github information found (name).")
            return None

        html_url = item.get('html_url')
        if html_url is None:
            self.log.debug("Incomplete Github information found (url).")
            return None

        description = item.get('description')
        if description is None:
            self.log.debug("Incomplete Github information found (description).")
            return None
    ...
    def handleEvent(self, event):
        ...
        if eventName == "SOCIAL_MEDIA":
            ...
            if network != "Github":
                self.log.debug(f"Skipping social network profile, {url}, as not a GitHub profile")
                return
            ...
            try:
                ...
                self.log.debug(f"Couldn't get a username out of {url}")
                return
            ...
            if not json_data.get('login'):
                self.log.debug(f"{username} is not a valid GitHub profile")
                return
            ...
            if len(location) < 3 or len(location) > 100:
                self.log.debug(f"Skipping likely invalid location: {location}")
                return
        ...
        self.log.debug(f"Looking at {username}")
        failed = False
        ...
        if not failed:
            try:
                ...
                self.log.debug(f"Error processing JSON response from GitHub: {e}")
                ret = None

            if ret is None:
                self.log.error(f"Unable to process empty response from Github for: {username}")
                failed = True

        if not failed:
            if ret.get('total_count', "0") == "0" or len(ret['items']) == 0:
                self.log.debug(f"No Github information for {username}")
                failed = True
        ...
        if not failed:
            try:
                ...
                self.log.error(f"Unable to process invalid response from Github for: {username}")
                failed = True

        if not failed:
            if ret.get('total_count', "0") == "0" or len(ret['items']) == 0:
                self.log.debug("No Github information for " + username)
                failed = True
        ...
        if not failed:
            ...
            for item in ret['items']:
                if item.get('repos_url') is None:
                    self.log.debug("Incomplete Github information found (repos_url).")
                    continue
                ...
                try:
                    ...
                    self.log.error(f"Invalid JSON returned from Github: {e}")
                    continue
                ...
                for item in repret:
                    if type(item) != dict:
                        self.log.debug("Encountered an unexpected or empty response from Github.")
                        continue
