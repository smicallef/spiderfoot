...
class sfp_shodan(SpiderFootPlugin):
    ...
    def query(self, qry):
        res = self.sf.fetchUrl(
            "https://api.shodan.io/shodan/host/" + qry + "?key=" + self.opts["api_key"],
    ...
    def searchHosts(self, qry):
        res = self.sf.fetchUrl(
            "https://api.shodan.io/shodan/host/search?query=hostname:"
            + qry
            + "&key="
            + self.opts["api_key"],
            timeout=self.opts["_fetchtimeout"],
    ...
    def searchHtml(self, qry):
        ...
        res = self.sf.fetchUrl(
            "https://api.shodan.io/shodan/host/search?" + urllib.parse.urlencode(params),
            timeout=self.opts["_fetchtimeout"],
            useragent="SpiderFoot",
        )
        if res["content"] is None:
    ...
    def handleEvent(self, event):
        ...
        if eventName == "WEB_ANALYTICS_ID":
            try:
                ...
                self.sf.error(
                    "Unable to parse WEB_ANALYTICS_ID: " + eventData + " (" + str(e) + ")", False
                )
                return None
        ...
        for addr in qrylist:
            ...
            if rec.get("os") is not None:
                ...
                evt = SpiderFootEvent(
                    "OPERATING_SYSTEM", rec.get("os") + " (" + addr + ")", self.__name__, pevent
                )
                self.notifyListeners(evt)
            ...
            if rec.get("devtype") is not None:
                # Notify other modules of what you've found
                evt = SpiderFootEvent(
                    "DEVICE_TYPE", rec.get("devtype") + " (" + addr + ")", self.__name__, pevent
                )
                self.notifyListeners(evt)
            ...
            if "data" in rec:
                ...
                for r in rec["data"]:
                    ...
                    if asn is not None:
                        evt = SpiderFootEvent(
                            "BGP_AS_MEMBER", asn.replace("AS", ""), self.__name__, pevent
                        )
                        self.notifyListeners(evt)
