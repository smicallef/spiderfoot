...
class sfp_apility(SpiderFootPlugin):
    ...
    # Default options
    opts = {"api_key": "", "delay": 2, "malicious_freemail": True}
    ...
    def queryBadDomain(self, qry):
        headers = {"Accept": "application/json", "X-Auth-Token": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.apility.net/baddomain/" + qry,
            headers=headers,
            timeout=15,
    ...
    def queryBadIp(self, qry):
        headers = {"Accept": "application/json", "X-Auth-Token": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.apility.net/badip/" + qry,
            headers=headers,
            timeout=15,
            useragent=self.opts["_useragent"],
        )
    ...
    def queryIp(self, qry):
        headers = {"Accept": "application/json", "X-Auth-Token": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.apility.net/v2.0/ip/" + qry,
            headers=headers,
            timeout=15,
            useragent=self.opts["_useragent"],
        )
    ...
    def queryAsIp(self, qry):
        headers = {"Accept": "application/json", "X-Auth-Token": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.apility.net/v2.0/as/ip/" + qry,
            headers=headers,
            timeout=15,
            useragent=self.opts["_useragent"],
        )
        ...
        if eventName == "IP_ADDRESS":
            ...
            for m in res:
                if m == "FREEMAIL" and not self.opts["malicious_freemail"]:
                    continue
                evt = SpiderFootEvent(
                    "MALICIOUS_IPADDR", m + " [" + eventData + "]", self.__name__, event
                )
            ...
            if domain.get("blacklist"):
                for m in domain.get("blacklist"):
                    if m == "FREEMAIL" and not self.opts["malicious_freemail"]:
                        continue
                    evt = SpiderFootEvent(
                        "MALICIOUS_INTERNET_NAME", m + " [" + eventData + "]", self.__name__, event
                    )
                    self.notifyListeners(evt)
