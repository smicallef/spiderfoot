...
class sfp_clearbit(SpiderFootPlugin):
    ...
    def query(self, t):
        ...
        if res["code"] != "200":
            self.sf.error(
                "Return code indicates no results or potential API key failure or exceeded limits.",
                False,
            )
            return None
        ...
        try:
            ...
            self.sf.error("Error processing JSON response from clearbit.io: " + str(e), False)
            return None
    ...
    def handleEvent(self, event):
        ...
        try:
            ...
            if "person" in data:
                ...
                evt = SpiderFootEvent(
                    "RAW_RIR_DATA", "Possible full name: " + name, self.__name__, event
                )
