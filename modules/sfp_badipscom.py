...
class sfp_badipscom(SpiderFootPlugin):
    """badips.com:Investigate,Passive:Reputation Systems::Check if a domain or IP is malicious according to badips.com."""

    # Default options
    opts = {"_badips": True, "checkaffiliates": True, "checkcohosts": True, "cacheperiod": 18}
        ...
        for check in list(malchecks.keys()):
            ...
            if id == cid and malchecks[check]["type"] == "query":
                ...
                if res["content"] is None:
                    ...
                    return None
                if self.contentMalicious(
                    res["content"], malchecks[check]["goodregex"], malchecks[check]["badregex"]
                ):
                    ...
                    for ip in iplist:
                        ...
                        try:
                            if IPAddress(ip) in IPNetwork(target):
                                self.sf.debug(
                                    ip + " found within netblock/subnet " + target + " in " + check
                                )
                                return url
                ...
                if "regex" not in malchecks[check]:
                    for line in data["content"].split("\n"):
                        if line == target or (targetType == "domain" and line == targetDom):
                            self.sf.debug(
                                target + "/" + targetDom + " found in " + check + " list."
                            )
                            return url
                else:
                    ...
                    try:
                        ...
                        for line in data["content"].split("\n"):
                            if (
                                targetType == "domain" and re.match(rxDom, line, re.IGNORECASE)
