...
class sfp_ssltools(SpiderFootPlugin):
    ...
    # Default options
    opts = {"verify": True, "certexpiringdays": 30}
    ...
    def queryDns(self, domain):
        params = {"url": domain.encode("raw_unicode_escape").decode("ascii", errors="replace")}

        headers = {"Content-Type": "application/x-www-form-urlencoded"}

        res = self.sf.fetchUrl(
            "http://www.ssltools.com/api/dns",
    ...
    def queryScan(self, domain, port):
        ...
        res = self.sf.fetchUrl(
            "http://www.ssltools.com/api/scan",
            postData=urllib.parse.urlencode(params),
            headers=headers,
    ...
    def handleEvent(self, event):
        ...
        if eventName != "IP_ADDRESS" and cert.get("mismatch"):
            evt = SpiderFootEvent(
                "SSL_CERTIFICATE_MISMATCH", ", ".join(cert.get("hosts")), self.__name__, event
            )
            self.notifyListeners(evt)
        ...
        if cert.get("expired"):
            evt = SpiderFootEvent(
                "SSL_CERTIFICATE_EXPIRED", cert.get("expirystr", "Unknown"), self.__name__, event
            )
            self.notifyListeners(evt)
            return None
        ...
        if cert.get("expiring"):
            evt = SpiderFootEvent(
                "SSL_CERTIFICATE_EXPIRING", cert.get("expirystr", "Unknown"), self.__name__, event
            )
            self.notifyListeners(evt)
            return None
