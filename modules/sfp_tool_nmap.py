...
class sfp_tool_nmap(SpiderFootPlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "sfp_tool_nmap":
            self.log.debug("Skipping event from myself.")
            return
        ...
        try:
            if eventName == "NETBLOCK_OWNER" and self.opts['netblockscan']:
                ...
                if net.prefixlen < self.opts['netblockscanmax']:
                    self.log.debug("Skipping port scanning of " + eventData + ", too big.")
                    return

        except Exception as e:
            self.log.error("Strange netblock identified, unable to parse: " + eventData + " (" + str(e) + ")")
            return
        ...
        if eventData in self.results:
            ...
            for addr in self.results:
                if IPNetwork(eventData) in IPNetwork(addr):
                    self.log.debug("Skipping " + eventData + " as already within a scanned range.")
                    return

        self.results[eventData] = True

        if not self.opts['nmappath']:
            self.log.error("You enabled sfp_tool_nmap but did not set a path to the tool!")
        ...
        if self.opts['nmappath'].endswith('nmap'):
            ...
            self.log.error("Could not recognize your nmap path configuration.")
            self.errorState = True
        ...
        try:
            ...
            if p.returncode == 0:
                ...
                self.log.error("Unable to read Nmap content.")
                self.log.debug(f"Error running Nmap: {stderr}, {stdout}")
                return

            if "No exact OS matches for host" in content or "OSScan results may be unreliable" in content:
                self.log.debug(f"Couldn't reliably detect the OS for {eventData}")
                return
        except Exception as e:
            self.log.error(f"Unable to run Nmap: {e}")
            return

        if not content:
            self.log.debug("No content from Nmap to parse.")
            return
        ...
        if eventName == "IP_ADDRESS":
            try:
                ...
                self.log.error("Couldn't parse the output of Nmap: " + str(e))
                return
        ...
        if eventName == "NETBLOCK_OWNER":
            try:
                ...
                self.log.error(f"Couldn't parse the output of Nmap: {e}")
                return
