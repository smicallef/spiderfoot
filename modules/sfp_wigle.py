...
class sfp_wigle(SpiderFootPlugin):
    ...
    def getcoords(self, qry):
        url = "https://api.wigle.net/api/v2/network/geocode?" + urllib.parse.urlencode(
        ...
        res = self.sf.fetchUrl(url, timeout=30, useragent="SpiderFoot", headers=hdrs)
    ...
    def getnetworks(self, coords):
        url = (
            "https://api.wigle.net/api/v2/network/search?onlymine=false&"
            + "latrange1="
            + str(coords[0])
            + "&latrange2="
            + str(coords[1])
            ...
            + "&longrange2="
            + str(coords[3])
            + "&freenet=false&paynet=false&variance="
            + self.opts["variance"]
        )
