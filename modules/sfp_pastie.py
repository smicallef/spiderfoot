...
class sfp_pastie(SpiderFootPlugin):
    """Pastie:Pastie scraping (via Google) to identify related content."""
    ...
    def producedEvents(self):
        return ["SEARCH_ENGINE_WEB_CONTENT", "PASTIE_CONTENT"]
    ...
    def handleEvent(self, event):
        ...
        pages = self.sf.googleIterate("site:pastie.org +\"" + eventData + "\"",
                                      dict(limit=self.opts['pages'],
                                           useragent=self.opts['_useragent'], timeout=self.opts['_fetchtimeout']))

        if pages is None:
            self.sf.info("No results returned from Google Pastie.org search.")
            return None
        ...
        for page in pages.keys():
            ...
            links = self.sf.parseLinks(page, pages[page], "pastie.org")
            if len(links) == 0:
                continue
            ...
            for link in links:
                ...
                if self.sf.urlBaseUrl(link).endswith("pastie.org"):
                    if self.checkForStop():
                        return None
                    ...
                    # Sometimes pastie search results false positives
                    ...
                    try:
                        ...
                        self.sf.debug("String not found in pastie.org content.")
                        continue

                    data = res['content'][startIndex:endIndex]

                    evt = SpiderFootEvent("PASTIE_CONTENT",
                                          "<SFURL>" + link + "</SFURL>\n" + "\"... " + data + " ...\"",
                                          self.__name__, event)
                    self.notifyListeners(evt)
