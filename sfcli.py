...
class SpiderFootCli(cmd.Cmd):
    ...
    def do_start(self, line):
        """start <target> <-m module1,module2,...>|<-t type1,type2>|<-u "Use case"> [-n name]
        Start a scan against <target> using modules or looking for types or by Use case ("all",
        "investigate", "passive" and "footprint").
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        ...
        helpmap = [
            ...
            ["start <target> [-m m1,...] [-t t1,...] [-u name] [-n name]", "Start a new scan."],
...
if __name__ == "__main__":
    ...
    p.add_argument("-p", metavar="PASS", type=str, help="Password to authenticate to SpiderFoot server. Consider using -g PASSFILE instead so that your password isn't visible in your shell history or in process lists!")
    p.add_argument("-P", metavar="PASSFILE", type=str, help="File oontaining password to authenticate to SpiderFoot server. Ensure permissions on the file are set appropriately!")
