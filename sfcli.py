...
class SpiderFootCli(cmd.Cmd):
    ...
    def request(self, url, post=None):
        ...
        try:
            headers = {"User-agent": "SpiderFoot-CLI/" + self.version}
    ...
    def do_ping(self, line):
        ...
        if s[1] != self.version:
            self.edprint(
                "Server and CLI version are not the same ("
                + s[1]
                + " / "
                + self.version
    ...
    def do_logs(self, line):
        ...
        if "-w" not in c[0]:
            d = self.request(
                self.ownopts["cli.server_baseurl"] + "/scanlog", post={"id": sid, "limit": limit}
            )
            if not d:
                return
            j = json.loads(d)
            ...
            self.send_output(
                d, line, titles={"0": "Generated", "1": "Type", "2": "Source", "3": "Message"}
            )
            return
        else:
            # Get the rowid of the latest log message
            d = self.request(
                self.ownopts["cli.server_baseurl"] + "/scanlog", post={"id": sid, "limit": "1"}
            )
            if not d:
                return
            j = json.loads(d)
    ...
    def do_stop(self, line):
        ...
        d = self.request(self.ownopts["cli.server_baseurl"] + "/stopscan?id=" + id + "&cli=1")
        if not d:
            return
        ...
        if s[0] == "SUCCESS":
            self.dprint(
                "Successfully requested scan to stop. This could take some minutes to complete."
            )
        else:
    ...
    def do_summary(self, line):
        ...
        d = self.request(
            self.ownopts["cli.server_baseurl"] + "/scansummary?id=" + sid + "&by=type"
        )
        if not d:
            return
        j = json.loads(d)
    ...
    def do_delete(self, line):
        ...
        d = self.request(
            self.ownopts["cli.server_baseurl"] + "/scandelete?confirm=1&raw=1&id=" + id
        )
        if not d:
            return
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        ...
        self.send_output(
            json.dumps(helpmap), "", titles={"0": "Command", "1": "Description"}, total=False
        )
    ...
    def do_set(self, line):
        ...
        if not cfg or not val:
            ...
            if len(output) > 0:
                self.send_output(
                    json.dumps(output), line, {"opt": "Option", "val": "Value"}, total=False
                )
            return
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "-u", metavar="USER", type=str, help="Username to authenticate to SpiderFoot server."
    )
    p.add_argument(
        "-p",
        metavar="PASS",
