    ...
    def handleEvent(self, event):
        ...
        if eventName == "TARGET_WEB_CONTENT":
            ...
            for m in matches:
                ...
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Google Site Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                ...
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Ahrefs Site Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
        ...
        if eventName == "DNS_TEXT":
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "LogMeIn Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "DocuSign Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "GlobalSign Site Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Atlassian Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Adobe IDP Site Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Adobe Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Stripe Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "TeamViewer SSO Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Aliyun Site Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Facebook Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Citrix Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Dropbox Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Detectify Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Drift Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Zoom.us Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Mail.ru Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Yandex Domain Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Brave Ledger Verification: " + m, self.__name__, event
                )
                self.notifyListeners(evt)
