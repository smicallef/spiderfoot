...
class sfp_searchcode(SpiderFootPlugin):
    ...
    def query(self, qry, page=1, per_page=100):
        ...
        if res['code'] == "429":
            self.log.error("You are being rate-limited by searchcode.")
            self.errorState = True
            return None

        if res['code'] != '200':
            self.log.error(f"Unexpected reply from searchcode: {res['code']}")
            self.errorState = True
            return None
        ...
        try:
            ...
            self.log.debug(f"Error processing JSON response from searchcode: {e}")
            return None
        ...
        while page < max_pages:
            ...
            for email in emails:
                ...
                if not self.getTarget().matches(mail_domain):
                    self.log.debug(f"Skipped email address: {email}")
                    continue
